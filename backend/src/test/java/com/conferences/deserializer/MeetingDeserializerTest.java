...
public class MeetingDeserializerTest {

    private static MeetingDeserializer deserializer;
    private static DeserializationContext context;
    private static JsonParser jsonParser;

    @BeforeAll
    private static void beforeAll() throws IOException {
        deserializer = new MeetingDeserializer(getDateHandlerMock());
        context = Mockito.mock(DeserializationContext.class);
        configureJsonParserMock();
    }

    @Test
    public void shouldDeserializeJsonToMeeting() throws IOException {
        Meeting meeting = deserializer.deserialize(jsonParser, context);

        assertEquals(6, meeting.getId());
        assertEquals("Meeting title", meeting.getTitle());
        assertEquals("Meeting description", meeting.getDescription());
    ...
    private static JsonNode configureJsonNode() {
        ...
        Mockito.when(titleNode.textValue()).thenReturn("Meeting title");
        Mockito.when(node.get("title")).thenReturn(titleNode);

        JsonNode descriptionNode = Mockito.mock(JsonNode.class);
        Mockito.when(descriptionNode.textValue()).thenReturn("Meeting description");
        Mockito.when(node.get("description")).thenReturn(descriptionNode);
